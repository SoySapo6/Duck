...
async function getRcanal() {
    ...
    try {
        ...
        const channelJids = Object.values(global.ch || {}).filter(v => v.endsWith('@newsletter'));
        if (!channelJids.length) return {};

        const newsletterJid = channelJids[Math.floor(Math.random() * channelJids.length)];
        ...
        return {
            contextInfo: {
                ...
                forwardedNewsletterMessageInfo: {
                    newsletterJid: name_canal,
                    serverMessageId: 100,
                    newsletterName: id_canal,
                }
            }
        };
    } catch (e) {
